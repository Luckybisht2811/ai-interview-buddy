...
function Step1SetUp({ onStart }) {
    ...
    const [questionCount, setQuestionCount] = useState(20); // Medium by default
    ...
    const handleUploadResume = async () => {
        ...
        try {
            ...
            setResumeData(result.data.resumeData || null);
    ...
    const handleStart = async () => {
        setLoading(true)
        try {
           const result = await axios.post(ServerUrl + "/api/interview/start" , {role, experience, mode , resumeText, projects, skills, resumeData, questionCount } , {withCredentials:true}) 
           console.log(result.data)
           if(userData){
            ...
            <div className='w-full max-w-6xl bg-white rounded-3xl shadow-2xl grid md:grid-cols-2 overflow-hidden'>
                ...
                <motion.div
                    ...
                    <div className='space-y-6'>
                        ...
                        <div>
                            <p className='text-sm text-gray-500 mb-2'>Interview length</p>
                            <div className='grid grid-cols-3 gap-3'>
                                {[
                                    { label: "Short", count: 15, sub: "~15 Qs" },
                                    { label: "Medium", count: 20, sub: "~20 Qs" },
                                    { label: "Long", count: 28, sub: "~28 Qs" },
                                ].map((opt) => (
                                    <button
                                        type="button"
                                        key={opt.label}
                                        onClick={() => setQuestionCount(opt.count)}
                                        className={`py-3 rounded-xl border text-sm font-medium transition ${questionCount === opt.count
